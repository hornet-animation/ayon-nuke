...
class CollectNukeWrites(
    ...
    def _get_collected_frames(self, instance):
        ...
        # local NTFS, but arbitrary on network/SAN shares like X:), so without
        # this the frame list can come back out of order -- and the earlier
        # sorted() above is discarded because it feeds a set. Downstream
