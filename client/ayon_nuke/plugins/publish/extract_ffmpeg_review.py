...
class ExtractFFmpegReview(
    ...
    # Prerenders never get review media (there's no review toggle on the
    # Quick PreWrite node), so exclude "prerender" here -- gating on the
    # review flag isn't enough because prerender bakes review=True at
    # creation and this plugin defaults a missing flag to True.
    families = ["render"]
    hosts = ["nuke"]
    targets = ["local", "farm"]
